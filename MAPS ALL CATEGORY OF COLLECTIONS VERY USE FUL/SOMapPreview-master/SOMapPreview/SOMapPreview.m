...
@property (nonatomic, strong) MKPolyline *polylineCache;
...
- (void)setRouteColor:(UIColor *)routeColor
{
	...
	if (_polylineCache)
		[self renderPolylineOnMap:_polylineCache];
}
...
- (NSUInteger)hashForPolyline:(MKPolyline *)polyline
{
	...
	[self.routeColor getRed:&red green:&green blue:&blue alpha:&alpha];
	/* Weight the color values so primary colors dont identify as similarly polar values */
	hash += ((red * 1) + (green * 2) + (blue * 3) + (alpha * 4)) * 255;
	...
	dispatch_async(waitQueue, ^{
		...
		[snapshotter startWithQueue:continueQueue completionHandler:^(MKMapSnapshot *snapshot, NSError *error) {
			...
			else
			{
				...
				newImage = [newImage drawOverlay:polyline snapshot:snapshot color:_routeColor];
